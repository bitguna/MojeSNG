...
public class EasySplashScreen {
    ...
    private int mLogo = 0;
    private Class<?> MainAcitvity = null;
    private int SPLASH_TIME_OUT = 0; //The time before launch target Activity - by default 2 seconds
    ...
    private void setUpHandler(){
        if (MainAcitvity != null) {
            new Handler().postDelayed(new Runnable() {
                @Override
                public void run() {
                    Intent i = new Intent(mActivity, MainAcitvity);
                    if (bundle != null) {
                        i.putExtras(bundle);
                    }
